// -----------------------------------------------------------
// EMOTION + HAND-DRIVEN PARTICLE SYSTEM FOR TEDxCMU
// -----------------------------------------------------------
// This version uses camera input for emotion detection AND hand tracking
// Requires: Processing Video library, oscP5 library
...
float handX = -1;
float handY = -1;
float handOpenness = 0.5;  // 0 = closed fist, 1 = open hand
boolean handDetected = false;
float handInfluenceRadius = 200;  // How far hand affects particles
...
void setup() {
  ...
  println("Listening for emotion + hand data on port 12000...");
  ...
  println("Press 'h' to simulate hand at mouse position (open)");
  println("Press 'j' to simulate closed hand");
  println("Press 'i' to toggle hand indicator");
}
...
void draw() {
  ...
  // NEW: Apply hand forces before running flock
  applyHandForces();
  ...
  // NEW: Draw hand indicator
  if (showHandIndicator) {
    drawHandIndicator();
  }
  
  // Display current emotion and hand info
  drawInfo();
}

boolean showCameraPreview = false;
boolean showHandIndicator = true;
...
void drawInfo() {
  ...
  // Hand info
  if (handDetected) {
    String handState = handOpenness > 0.6 ? "OPEN (attract)" : 
                       handOpenness < 0.3 ? "CLOSED (repel)" : 
                       "NEUTRAL";
    text("Hand: " + handState, 10, 50);
  } else {
    text("Hand: Not detected", 10, 50);
  }
}

// -----------------------------------------------------------
// HAND FORCE APPLICATION
// -----------------------------------------------------------

void applyHandForces() {
  if (!handDetected) return;
  
  for (Boid b : flock.boids) {
    float dist = dist(b.position.x, b.position.y, handX, handY);
    
    // Only affect particles within influence radius
    if (dist < handInfluenceRadius && dist > 1) {
      PVector force = new PVector(handX - b.position.x, handY - b.position.y);
      force.normalize();
      
      // Hand openness determines behavior
      if (handOpenness > 0.6) {
        // Open hand = ATTRACT particles
        float strength = map(dist, 0, handInfluenceRadius, 0.8, 0.1);
        force.mult(strength);
        b.acceleration.add(force);
      } else if (handOpenness < 0.3) {
        // Closed hand = REPEL particles
        force.mult(-1);  // Reverse direction
        float strength = map(dist, 0, handInfluenceRadius, 1.2, 0.2);
        force.mult(strength);
        b.acceleration.add(force);
      }
      // Medium openness (0.3-0.6) = minimal effect (transition zone)
    }
  }
}
...
void drawHandIndicator() {
  ...
  stroke(255, 100);
  strokeWeight(2);
  circle(handX, handY, handInfluenceRadius * 2);
  
  // Draw hand position dot with color based on state
  if (handOpenness > 0.6) {
    fill(0, 255, 0, 150);  // Green = attract
  } else if (handOpenness < 0.3) {
    fill(255, 0, 0, 150);  // Red = repel
  } else {
    fill(255, 255, 0, 150);  // Yellow = neutral
  }
  noStroke();
  circle(handX, handY, 30);
  
  // Draw openness percentage
  fill(255);
  textAlign(CENTER, CENTER);
  textSize(12);
  text(nf(handOpenness * 100, 0, 0) + "%", handX, handY);
...
void keyPressed() {
  ...
  // Toggle hand indicator
  if (key == 'i' || key == 'I') {
    showHandIndicator = !showHandIndicator;
  }
  
  // Manual hand testing (simulate hand at mouse position)
  if (key == 'h' || key == 'H') {
    handX = mouseX;
    handY = mouseY;
    handOpenness = 1.0;  // Open hand
    handDetected = true;
    println("Simulating OPEN hand at mouse position");
  }
  
  if (key == 'j' || key == 'J') {
    handX = mouseX;
    handY = mouseY;
    handOpenness = 0.0;  // Closed hand
    handDetected = true;
    println("Simulating CLOSED hand at mouse position");
  }
...
void oscEvent(OscMessage msg) {
  ...
  // NEW: Hand data from Python
  if (msg.checkAddrPattern("/hand")) {
    handX = msg.get(0).floatValue() * width;  // Convert 0-1 to screen coords
    handY = msg.get(1).floatValue() * height;
    handOpenness = msg.get(2).floatValue();
    handDetected = true;
    
    // Optional: Print to console for debugging
    // println("Hand: " + handX + ", " + handY + " | Open: " + nf(handOpenness, 0, 2));
  }
}
